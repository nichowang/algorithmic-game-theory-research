...
for sv in tqdm(share_vectors, desc='CEL scenarios'):
    ...
    orig_aw = awards_from_matrix(R_truth, estate=70)

    for m in range(len(sv)):
        rest_best, rest_rows, rest_sh = all_opt_rows_restricted(R_truth, m, estate=70)
        unb_best,  unb_rows           = all_opt_rows_unbounded(R_truth, m, estate=70)
