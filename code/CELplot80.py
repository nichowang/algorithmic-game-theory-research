...
def build_restricted_id_cache(R0, manip_index, eps=1e-9):
    ...
    S_b_mask      = np.zeros((n, n), dtype=bool)   # [b, l]
    base_cnt_b    = np.zeros(n, dtype=np.int32)
    sums_by_a     = np.zeros((n, n), dtype=float)  # sum_l R[l,a]/R[l,b]
...
def impartial_division_restricted_fast(R, cache):
    """
    Same math as original 'impartial_division', but uses the cache built above.
    R differs from cache['R'] only at row m; others are identical.
    """
    ...
    for j in range(n):
        ...
        for i in range(n):
            ...
            base_sum_i = base_sum_psi[i, :]   # view
            inv_vec    = inv_col_i[i, :]      # view
            Rmi        = Rm[i]                # R[m,i]
...
def all_opt_rows_restricted(R0, m, estate=80, scale=100, tol=1e-9, alpha=0.01, pbar=None, MAX_KEEP=32):
...
def all_opt_rows_unrestricted(R0, m, estate=80, scale=100, tol=1e-9, alpha=0.01, pbar=None, MAX_KEEP=32):
    n = R0.shape[0]
    fixed  = float(R0[m, m])
    others = [i for i in range(n) if i != m]
    k = len(others)
...
def suggested_entitlements(n, manip=0, E=80.0, D=100.0, foil=None):
...
for n in NS_TRACK:
    ...
    pbar_r = tqdm(total=total_candidates, desc=f"[n={n}] restricted", unit="cand", leave=False)
    rest_best, _, _ = all_opt_rows_restricted(
        R_truth, AGENT_M, estate=E_FIXED, scale=SCALE, alpha=ALPHA, pbar=pbar_r
    )

    pbar_u = tqdm(total=total_candidates, desc=f"[n={n}] unrestricted", unit="cand", leave=False)
