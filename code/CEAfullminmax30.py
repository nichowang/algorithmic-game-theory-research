...
def restricted_best(R0, m, estate=30, scale=100, alpha=0.01, tol=1e-9):
    n = R0.shape[0]
    fixed = float(R0[m, m])
...
def unrestricted_best(R0, m, estate=30, scale=100, alpha=0.01, tol=1e-9):
    n = R0.shape[0]
    fixed = float(R0[m, m])
    others = [i for i in range(n) if i != m]
    k = len(others)
...
def sweep_minmax_differences_with_progress(E=30, D=100, alpha=0.01, manip_index=0):
    n = 4
...
if __name__ == "__main__":
    ...
    plt.title(f"CEA – Max-Min Differences vs a0 (E={E}, D={D}, n=4)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig("cea_sweep_a0_minmax_differences.png", dpi=200)
    plt.show()