# =================== CG-CEA version (CEA under d_i/2 caps) ===================
...
# -------------------- 1) Base CEA (unchanged) --------------------
def cea_allocation_base(claims, estate, iters=60):
    """
    Standard CEA on raw 'claims'.
    """
...
# -------------------- CG-CEA wrapper --------------------
def cg_cea_allocation(demands_d, estate, iters=60):
    """
    Contested Garment (CG) interpretation of CEA:
    - Run CEA under per-agent caps of d_i/2.
    - Equivalently: run standard CEA on the 'claims' vector = d/2.
    - This ensures the final allocation never exceeds d_i/2 for any i.
    """
    d = np.asarray(demands_d, dtype=np.float64)
    capped_claims = 0.5 * d
    return cea_allocation_base(capped_claims, estate, iters=iters)

def truthful_matrix(vec):
    vec = np.asarray(vec, dtype=np.float64)
    return np.tile(vec, (len(vec), 1))

def awards_from_matrix_CG_CEA(R, estate=35, scale=100):
    """
    Given a rating matrix R, form demands d = mean-column * scale,
    then apply CG-CEA (CEA on d/2).
    """
    d = R.mean(axis=0) * scale
    return cg_cea_allocation(d, estate)

# -------- Optimized impartial_division with numba (unchanged) --------
@njit(cache=True)
...
def all_opt_rows_restricted(R0, m, estate=20, scale=100, alpha=0.01, pbar=None):
    """
    Restricted: manipulate row m in R, then:
      - compute impartial-division shares on the *full* matrix
      - transform to demands d = shares * scale
      - apply CG-CEA on d (CEA on d/2)
    """
    n = R0.shape[0]
    fixed = float(R0[m, m])
    ...
    for add_units in compositions_nonneg_iter(free_units, k, out_arr=buf):
        ...
        d = shares * scale                    # demands under this R
        awards = cg_cea_allocation(d, estate) # CG-CEA
        val = awards[m]
...
def all_opt_rows_unbounded(R0, m, estate=20, scale=100, alpha=0.01, pbar=None):
    """
    Unbounded: manipulate row m, and directly feed the row as 'demands' d=row*scale
    into CG-CEA (no impartial-division recompute).
    """
    n = R0.shape[0]
    fixed = float(R0[m, m])
    others = np.array([i for i in range(n) if i != m])
    ...
    for add_units in compositions_nonneg_iter(free_units, k, out_arr=buf):
        ...
        d = row * scale                       # manipulated demands
        awards = cg_cea_allocation(d, estate) # CG-CEA
        val = awards[m]
...
if __name__ == "__main__":
    ...
    print("n | manip awards (Original / Restricted / Unrestricted)  [CG-CEA]")
    print("-" * 78)

    for n in NS_TRACK:
        # Same synthetic 'demands' vector as before
        claims = [E_FIXED / n] * (n - 1) + [D_BIG]
        m = n - 1
        vec = np.array(claims, dtype=np.float64) / SCALE
        R_truth = truthful_matrix(vec)

        # Original line now uses CG-CEA
        awards_orig = awards_from_matrix_CG_CEA(R_truth, estate=E_FIXED, scale=SCALE)
        manip_orig = float(awards_orig[m])

        # progress sizes identical to before
        k = n - 1
        min_units = int(round(ALPHA * SCALE))
    ...
    # Save plot data
    print("\n=== SAVED DATA POINTS (n, Original, Restricted, Unrestricted) [CG-CEA] ===")
    ...
    plt.title("CG-CEA – manipulator payoff vs n  (test: [E/n,...,E/n, d_big])")
    plt.legend()
    plt.tight_layout()

    ns_tag = "-".join(map(str, ns))
    out_name = f"cg_cea_E{int(E_FIXED)}_dbig{int(D_BIG)}_ns{ns_tag}.png"
