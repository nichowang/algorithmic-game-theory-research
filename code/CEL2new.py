...
#  Core: CG-CEL (Contested Garment CEL - optimized with numba) 
@njit(cache=True)
def cel_allocation_numba(claims, estate, iters=60):
    """
    Standard CEL allocation helper 
    """
    n = len(claims)
    lo, hi = 0.0, 0.0
    for i in range(n):
...
if __name__ == "__main__":
    ...
    _ = cg_cel_allocation_numba(dummy_claims, 60.0)
    ...
    excel_filename = "cg_cel_sweep_results_full_E60.xlsx"
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("cg_cel_sweep_a0_unique_weighted_E60.png", dpi=200)
    print(f"\nPlot saved to cg_cel_sweep_a0_unique_weighted_E60.png")
    plt.show()
