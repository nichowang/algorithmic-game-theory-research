# =================== CG-CEA sweep (unique combos, weighted average) - OPTIMIZED ===================
import numpy as np
import itertools
...
# ---------- Core: CG-CEA (Contested Garment CEA - optimized with numba) ----------
@njit(cache=True)
def cg_cea_allocation_numba(claims, estate, iters=60):
    """
    Contested Garment CEA allocation
    Allocates at most claims[i]/2 to each agent i
    """
    n = len(claims)
    # Cap at half of each claim
    half_claims = np.empty(n, dtype=np.float64)
    for i in range(n):
        half_claims[i] = claims[i] * 0.5

    # Find lambda for CEA on half_claims
    lo, hi = 0.0, 0.0
    for i in range(n):
        if half_claims[i] > hi:
            hi = half_claims[i]
...
def restricted_best(R0, m, estate=40, scale=100, alpha=0.01):
    n = R0.shape[0]
    fixed = R0[m, m]
...
def unrestricted_best(R0, m, estate=40, scale=100, alpha=0.01):
    n = R0.shape[0]
    fixed = R0[m, m]
    others = np.array([i for i in range(n) if i != m], dtype=np.int32)
    k = len(others)
...
if __name__ == "__main__":
    ...
    _ = cg_cea_allocation_numba(dummy_claims, 40.0)
    ...
    excel_filename = "cg_cea_sweep_results_full_E40.xlsx"
    ...
    plt.ylabel("Final reward of manipulator (CG-CEA)")
    plt.title(
        f"CG-CEA – Manipulator Reward vs a0 (unique combos; E={E}, D={D}, n=4)\n"
        f"weighted_by_multiplicity={WEIGHT_BY_MULTIPLICITY}"
    )
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig("cg_cea_sweep_a0_unique_weighted_E40.png", dpi=200)
    print(f"\n✅ Plot saved to cg_cea_sweep_a0_unique_weighted_E40.png")
    plt.show()
