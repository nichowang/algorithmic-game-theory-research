...
def awards_from_matrix(R, estate=51, scale=100):
...
def all_opt_rows_restricted(R0, m, estate=51, scale=100, tol=1e-9, alpha=0.01, pbar=None):
    n = R0.shape[0]
    fixed  = float(R0[m, m])
...
def all_opt_rows_unrestricted(R0, m, estate=51, scale=100, tol=1e-9, alpha=0.01, pbar=None):
    n = R0.shape[0]
    fixed  = float(R0[m, m])
    others = [i for i in range(n) if i != m]
    k = len(others)
...
def suggested_entitlements(n, manip=0, E=51.0, D=100.0, foil=None):
