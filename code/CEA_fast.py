...
def run_experiment():
    ...
    print(f"CG-CEA Experiment (Fast Search)")
    ...
    for n in n_values:
        ...
        # Setup for maximum manipulability: unanimous vector for CEA
        # Formula: (2E/n, ..., 2E/n, D - 2(n-1)E/n)
        # claims: [2E/n, 2E/n, ..., 2E/n, D - 2(n-1)E/n]
        #                                 └ manipulator (biggest)
        ...
        print(f"  Restricted:    {rest_best:.6f} (raw: {rest_best})")

        # Unrestricted (fast search)
        unbd_best = fast_unrestricted_search(manip_share, E_FIXED, SCALE, ALPHA, n, manip_index=m)
        print(f"  Unrestricted:  {unbd_best:.6f} (raw: {unbd_best})")
        print(f"  Comparison: Orig={orig:.10f}, Rest={rest_best:.10f}, Unbd={unbd_best:.10f}")
...
def plot_results(results):
    ...
    plt.title('CG-CEA: Manipulator Payoff vs n (E=40, D=100)', fontsize=13)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.xticks(ns)
    plt.tight_layout()

    plt.savefig('CEA_fast_n4567.pdf', dpi=300, bbox_inches='tight')
    plt.savefig('CEA_fast_n4567.png', dpi=200, bbox_inches='tight')
    print(f"\n✅ Saved: CEA_fast_n4567.pdf & CEA_fast_n4567.png")
