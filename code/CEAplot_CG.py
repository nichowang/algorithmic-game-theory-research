# =================== CG-CEA: run + one-figure plot (3 lines, per-n tqdm progress) ===================
...
# -------- CG-CEA: Conditional Grant version of CEA --------
def cg_cea_allocation(claims, estate, iters=60):
    """
    CG-CEA: First run CEA on half the estate, then run CEA on the rest with remaining claims.
    """
    c = np.asarray(claims, dtype=np.float64)

    # Step 1: Run CEA on E/2
    half_estate = estate / 2.0
    first_allocation = cea_allocation(c, half_estate, iters)

    # Step 2: Compute remaining claims
    remaining_claims = c - first_allocation
    remaining_claims = np.maximum(remaining_claims, 0)

    # Step 3: Run CEA on remaining estate (E/2) with remaining claims
    second_allocation = cea_allocation(remaining_claims, half_estate, iters)
...
def all_opt_rows_restricted(R0, m, estate=20, scale=100, tol=1e-9, alpha=0.01, pbar=None):
    ...
    for add_units in compositions_nonneg_iter(free_units, k, out_arr=buf):
        ...
        val    = cg_cea_allocation(shares * scale, estate)[m]   # ← 用 CG-CEA 计算 payoff
...
def all_opt_rows_unbounded(R0, m, estate=20, scale=100, tol=1e-9, alpha=0.01, pbar=None):
    ...
    for add_units in compositions_nonneg_iter(free_units, k, out_arr=buf):
        ...
        val = cg_cea_allocation(row * scale, estate)[m]         # ← 用 CG-CEA 计算 payoff
...
# -------------------- 3) Test case: [2E/n,...,2E/n, d_big] for CG-CEA --------------------
E_FIXED   = 20
SCALE     = 100
ALPHA     = 0.01
# For CG-CEA, the unanimous vector is (2E/n, ..., 2E/n, D - 2(n-1)/n * E)
# D_BIG will be computed dynamically based on n
...
for n in NS_TRACK:
    # CG-CEA unanimous vector: (2E/n, ..., 2E/n, D - 2(n-1)/n * E)
    # We need to choose D such that the last agent's claim is positive
    # Let's use D_BIG = 60.0 as in the original, but compute the actual value
    D_TOTAL = 60.0  # Total budget/reference value
    d_big = D_TOTAL - 2 * (n - 1) * E_FIXED / n

    claims = [2 * E_FIXED / n] * (n - 1) + [d_big]
    m = n - 1
...
plt.title("CG-CEA – manipulator payoff vs n  (test case: [2E/n,...,2E/n, d_big])")
plt.legend()
plt.tight_layout()

ns_tag = "-".join(map(str, ns))
out_name = f"cg_cea_E{int(E_FIXED)}_ns{ns_tag}.png"
