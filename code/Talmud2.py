...
def restricted_best(R0, m, estate=51, scale=100, alpha=0.01, tol=1e-9):
    n = R0.shape[0]
    fixed = float(R0[m, m])
...
def unrestricted_best(R0, m, estate=51, scale=100, alpha=0.01, tol=1e-9):
    n = R0.shape[0]
    fixed = float(R0[m, m])
    others = [i for i in range(n) if i != m]
    k = len(others)
...
def sweep_unique_combinations_with_progress(E=51, D=100, alpha=0.01, manip_index=0):
    n = 4
